...
implicit none
integer, parameter::kmax=1000,n=4
double precision:: a(n,n), b(n), x(n),soln(n)
double precision:: sum, tolerance = 0.01, max_error = 100, error,prev_error,ratio
integer i, j, k

a=reshape((/4,3,2,1,3,4,3,2,2,3,4,3,1,2,3,4/),shape(a)) !given matrix initialization
b=reshape((/1,1,-1,-1/),shape(b))                       !given RHS 
x=reshape((/0,0,0,0/),shape(x))                         !initial guess of the solution
...
do k=1,kmax
    ...
    if(max_error < tolerance) then                  !to make error (||x-x^(k)||) less than tolerance
        print*,
        print*,'no. of iterations-',k-1             !prints total no of iterations
        exit
    end if
...
!gauss seidel method
    do i=1,n
        ...
        if(max_error < error) then                      !calculating max error 
            max_error = error
        end if
    end do
